...
function applyDrift(weather) {
  ...
  return {
    ...
    visibility_m:    Math.max(10, weather.visibility_m + (dTemp * 100)),
  };
}
...
function mockData(zone) {
  ...
  return {
    ...
    visibility_m:     Math.round(5000 + s * 5000),
    curfew_active:    0,
...
function simData(zone, type) {
  ...
  return {
    ...
    humidity_pct:     72,
    wind_kmh:         type === 'cyclone' ? 70 + s * 20 : 14,
    visibility_m:     type === 'fog'    ? 15 + s * 20 : 8000,
    fetched_at:       new Date().toISOString(),
    source:           'simulation',
  };
}
